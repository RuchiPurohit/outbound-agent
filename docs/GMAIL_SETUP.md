...
3. Configure Google Auth Platform's consent screen (branding and audience).
   ...
   and add `aiwithruchi@gmail.com` as a test user.
...
7. Copy `.env.example` to `.env` locally, then fill in `GOOGLE_CLIENT_ID` and
   `GOOGLE_CLIENT_SECRET` from that client. Keep the sender as
   `aiwithruchi@gmail.com`. Never paste credentials into chat or commit them.
...
3. Click **Connect Gmail**, choose `aiwithruchi@gmail.com`, and grant permission.
   The dashboard refuses a different sender account.
4. In the test form, verify the recipient is `abstract.ruch@gmail.com`, inspect

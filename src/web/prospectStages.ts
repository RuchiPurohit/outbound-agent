...
export function renderProspectStages(
  ...
  const emails = outreach.map((draft) => {
    ...
    return `<details class="email-draft" id="email-${draft.id}"${draft.id === selectedDraftId ? " open" : ""}><summary>${escapeHtml(contact.name)} · ${escapeHtml(company.name)} · ${escapeHtml(draft.subject)} <span class="badge ${draft.status}">${escapeHtml(draft.status.replaceAll("_", " "))}</span></summary><p><strong>To:</strong> ${escapeHtml(contact.email ?? "No email")} · ${escapeHtml(contact.title)}</p><p><strong>Subject:</strong> ${escapeHtml(draft.subject)}</p><pre class="email-preview">${escapeHtml(draft.body)}</pre>${signal ? `<p class="muted"><strong>Cited signal:</strong> ${escapeHtml(signal.signal)} <a href="${escapeHtml(signal.sourceUrl)}" target="_blank" rel="noreferrer">Source ↗</a></p>` : '<div class="notice error">This legacy draft has no linked prospect signal and cannot be approved. Reject it or complete research and request a rewrite.</div>'}${reviewControls}</details>`;
  }).join("");
